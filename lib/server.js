...
/**
 * Creates an instance of the Litecore Wallet Service.
 * @constructor
 */
...
/**
 * Gets the current version of LWS
 */
WalletService.getServiceVersion = function() {
  if (!serviceVersion)
    serviceVersion = 'lws-' + require('../package').version;
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  try {
    pubKey = new Litecore.PublicKey.fromString(opts.pubKey);
  } catch (ex) {
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  try {
    Litecore.HDPublicKey(opts.xPubKey);
  } catch (ex) {
...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['ltc', 'bit'], value.toLowerCase());
    },
  }];
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  var networkName = Litecore.Address(addresses[0]).toObject().network;
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    ...
    log.error('Error building Litecore transaction', ex);
    return ex;
  }

  if (bitcoreError instanceof Litecore.errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (bitcoreError instanceof Litecore.errors.Transaction.DustOutputs)
  ...
  function select(utxos, cb) {
    ...
    _.each(smallInputs, function(input, i) {
      ...
      if (netTotal >= txpAmount) {
        ...
        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Litecore.Transaction.DUST_AMOUNT);
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  ...
  for (var i = 0; i < opts.outputs.length; i++) {
    ...
    var toAddress = {};
    try {
      toAddress = new Litecore.Address(output.toAddress);
    } catch (ex) {
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function getNormalizedTxs(addresses, from, to, cb) {
    ...
    var network = Litecore.Address(addresses[0].address).toObject().network;
